    ...
    let call = (action: action): unit => {
      ...
      | Delete(request) =>
        Js.Promise.(
          ...
          |> then_(result => {
               switch (result) {
               | Ok(_resp) => Success("Deleted succeeded") |> resolve
      ...
      | Put(request) =>
        Js.Promise.(
          Fetcher.put(buildUrl(endpoint, request.qs), request.body)
          |> then_(result =>
               switch (result) {
               | Ok(json) =>
                 switch (json |> decoder) {
                 | Ok(decoded) => setState(_ => Success(decoded)) |> resolve
                 | Error(_) =>
                   setState(_ =>
                     Failure("Could not decode JSON data with the given encoder")
                   )
                   |> resolve
                 }
