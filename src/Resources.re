...
let resolveSR =
    (repos: list(repo), sr: SF.Project.sourceRepository)
...
let resolveProject =
    (default_tenant: string, repos: list(repo), project: SF.Project.project)
    : SF.Project.project => {
  {
    ...project,
    source_repositories:
    ...
    (default_tenant: string, resources: SF.Resources.top): SF.Resources.top => {
  {
    resources: {
      ...
      projects:
        ...
        ->Belt.List.map(resolveProject(default_tenant, [])),
    },
  };
};
